...
import com.vaadin.plugin.actions.InstallJetbrainsRuntimeAction.RevealJBRFileAction
...
object JetbrainsRuntimeUtil {
    ...
    fun findLatestJBRRelease(): GitHubRelease? {
        try {
            val urlStr = "$JETBRAINS_RELEASES_URL/latest"
            val text = HttpConnectionUtils.openConnection(urlStr).inputStream.bufferedReader().use { it.readText() }
