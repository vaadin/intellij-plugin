...
/**
 * Provides exclusive CSS class name completion for Vaadin Flow's HasStyle methods.
 * Only shows CSS classes, no other suggestions.
 */
class CssClassCompletionContributor : CompletionContributor() {
    ...
    private class VaadinCssCompletionProvider : CompletionProvider<CompletionParameters>() {
        ...
        override fun addCompletions(
            ...
            val allClassNames = StubIndex.getInstance()
                .getAllKeys(CssClassIndex.KEY, project)

            for (className in allClassNames) {
                val classes = StubIndex.getElements(
                    CssClassIndex.KEY, className, project, scope, CssSelectorSuffix::class.java
                )
    ...
    companion object {
        ...
        private val TARGET_METHODS = setOf(
            "addClassName", "addClassNames",
            "setClassName", "removeClassName", "removeClassNames"
        )
